  ...
  <AppLayout>
    <div class="page-body">
      ...
      <main class="page-content">
        <BrandFormCard title="Add Leopard Pickup Address" subtitle="Fill in the shipper details for Leopard bookings.">
          ...
          <div class="grid two">
            <div class="form-group">
              ...
              <select v-model.number="form.origin_city" class="form-input" :class="{ 'input-error': errors.origin_city }" :disabled="citiesLoading">
            ...
            <div class="form-group">
              ...
              <select v-model.number="form.return_city" class="form-input" :class="{ 'input-error': errors.return_city }" :disabled="citiesLoading">
                <option value="">{{ citiesLoading ? 'Loading cities...' : 'Select city' }}</option>
                <option v-for="city in cities" :key="city.id" :value="city.id">{{ city.name }}</option>
              </select>
          ...
          <div class="panel-actions">
            ...
            <button type="button" class="btn-save" :disabled="saving || citiesLoading" @click="handleSubmit">
...
const loadCities = async () => {
  ...
  try {
    const res = await LeopardService.fetchCities();
    cities.value = res.data.data.cities || [];
  } catch (error) {
    loadError.value = error.response?.data?.message || 'Unable to load Leopard cities.';
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    router.push('/leopard-pickup-addresses?toast=created');
  } catch (error) {
    ...
    if (data?.errors) {
      ...
      loadError.value = data?.message || 'Unable to save Leopard pickup address.';
    }
  } finally {
    saving.value = false;
  }
