      ...
      <section class="scan-panel">
        <header class="scan-header">
          <div>
            ...
            <p>Scan pending shipment tracking numbers to mark them as shipped.</p>
          </div>
          <RouterLink class="back-btn" to="/packing-logs/pending">Back to Pending</RouterLink>
        </header>
        ...
        <section class="scanner-panel">
          <div class="scanner-controls">
            <div>
              <h2>Scanner</h2>
              <p>Scanned Shipments For Dispatching ({{ scannedOrders.length }})</p>
            </div>
            <div class="scanner-input-group">
              ...
              <button class="scanner-submit-btn" type="button" :disabled="scannerLoading" @click="handleScannerSubmit">
                {{ scannerLoading ? 'Processing...' : 'Mark Shipped' }}
              </button>
            </div>
          </div>
            ...
            <table class="scanned-table">
              ...
              <tbody>
                <tr v-if="scannedOrders.length === 0">
                  <td colspan="4" class="empty-scanned-cell">No scanned shipments.</td>
                </tr>
...
const handleScannerSubmit = async () => {
  ...
  try {
    const response = await PackingLogService.markShippedByTracking(trackingNumber);
...
.scanner-input-group {
  display: grid;
  grid-template-columns: minmax(260px, 360px) 128px;
  gap: 10px;
  align-items: center;
}
