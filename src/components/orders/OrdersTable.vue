      ...
      <tbody>
        ...
        <tr v-else v-for="(order, index) in orders" :key="order.id" class="order-row">
          ...
          >
            ...
            <template v-else-if="column.key === 'status_category'">
              {{ order.status_category || '—' }}
            </template>
            <template v-else-if="column.key === 'courier'">
              {{ order.courier_name || '—' }}
            </template>
            <template v-else-if="column.key === 'cod'">
              {{ formatMoney(order.currency, order.cod) }}
            </template>
            <template v-else-if="column.key === 'advance_payment'">
              {{ formatMoney(order.currency, order.advance_payment) }}
            </template>
            <template v-else-if="column.key === 'shipping_price'">
              {{ formatMoney(order.currency, order.shipping_price) }}
            </template>
            <template v-else-if="column.key === 'product_skus'">
              <span class="truncate" :title="productSkus(order)">{{ productSkus(order) || '—' }}</span>
            </template>
            <template v-else-if="column.key === 'product_quantity'">
              {{ productQuantity(order) || '—' }}
            </template>
            <template v-else-if="column.key === 'last_saved_by'">
              {{ order.last_saved_by?.name || '—' }}
            </template>
            <template v-else-if="column.key === 'label_generated_by'">
              {{ order.label_generated_by?.name || '—' }}
            </template>
            <template v-else-if="column.key === 'label_generated_at'">
              {{ formatDate(order.label_generated_at) }}
            </template>
            <template v-else-if="column.key === 'status_updated_at'">
...
const columnDefinitions = [
  ...
  { key: 'cod', header: 'COD' },
  { key: 'advance_payment', header: 'Advance Payment' },
  { key: 'shipping_price', header: 'Shipping Price' },
  { key: 'product_skus', header: 'Product SKUs' },
  { key: 'product_quantity', header: 'Product Quantity' },
  { key: 'last_saved_by', header: 'Last Saved By' },
  { key: 'label_generated_by', header: 'Label Generated By' },
  { key: 'label_generated_at', header: 'Label Generated At' },
  { key: 'status_updated_at', header: 'Status Updated At' },
...
const normalizedLineItems = (order) => Array.isArray(order.line_items) ? order.line_items : [];
const productSkus = (order) => normalizedLineItems(order).map(item => item.sku).filter(Boolean).join(' | ');
const productQuantity = (order) => normalizedLineItems(order).reduce((total, item) => total + Number(item.quantity || 0), 0);
